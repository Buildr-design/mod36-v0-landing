...
// src/components/sections/core-philosophy-section.tsx
'use client';

import { motion, useInView } from 'framer-motion';
import { useRef } from 'react';
import type { CorePhilosophySectionContent } from '@/data/site-content';
...
export function CorePhilosophySection({ content }: { content?: CorePhilosophySectionContent }) {
  const sectionRef = useRef(null);
  const isInView = useInView(sectionRef, { once: true, amount: 0.2 });

  if (!content || !content.blocks) return null;

  const poeticizableText = content.blocks.map(block => `${block.title}\n${block.description}`).join('\n\n');
  ...
  return (
    <motion.section
      ...
      aria-labelledby="core-philosophy-title"
      initial="hidden"
    ...
    >
      ...
      >
        {content.blocks.map((block, index) => {
          const IconComponent = LucideIcons[block.iconName as keyof typeof LucideIcons] as LucideIcons.LucideIcon;
          return (
            <motion.div key={index} variants={blockVariants}>
              <Card className="bg-card/50 hover:bg-card/80 border-border/50 hover:border-primary/50 transition-all duration-300 h-full flex flex-col items-center text-center p-6 rounded-lg shadow-md hover:shadow-primary/20">
                <CardHeader className="items-center">
                  ...
                  <CardTitle className="text-xl font-semibold text-foreground group-hover:text-primary">{block.title}</CardTitle>
                </CardHeader>
